<?php
	...
	if (isset($_REQUEST['check'])) {
		echo $_REQUEST['check']." ".$_REQUEST['task_id'];
...
<div class="page-wrapper">
	...
	<?php include("pages/header.php") ?>
		...
		<?php include("pages/control_panel.php") ?>
			...
			if (!isset($_GET['show_completed']) or $_GET['show_completed'] == '0') {
				$query .= " AND is_done = 0";
			}
			if (isset($_GET['date'])) {
			...
			foreach ($tasks as $task) { ?>
				...
				<td class="task__select">
					...
					<input class="checkbox__input visually-hidden task__checkbox" 
						type="checkbox" value="<?php echo $task[0] ?>" <?php if ($task[6]) echo "checked"; ?>>
					<span class="checkbox__text"><?php echo $task[1] ?></span>
				    </label>
				</td>
				<td class="task__file">
			            <?php if ($task[5] != 'NULL') : ?>
					<a class="download-link" href="<?php echo '/uploads/'.$task[5] ?>"><?php echo $task[5] ?></a>
				    <?php endif ?>
				</td>
				<td class="task__date"><?php echo $task[4] ?></td>
			    </tr>
		    <?php } ?>
                </table>
            </main>
