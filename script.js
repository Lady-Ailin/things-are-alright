...
if ($taskCheckboxes.length) {
  $taskCheckboxes[0].addEventListener('change', function (event) {
    if (event.target.classList.contains('task__checkbox')) {
      ...
      var searchParams = new URLSearchParams(window.location.search);
      var url = '/index.php?task_id=' + task_id + '&check=' + Number(is_checked) + "&date=" + searchParams.get("date") +  "&show_completed=" + searchParams.get("show_completed")  +  "&project=" + searchParams.get("project");
      window.location = url;
    }
  });
}
